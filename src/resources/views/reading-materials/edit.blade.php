...
@section('title', 'Edit Reading Material')

@section('header', 'Edit Reading Material')
                ...
                <div class="row g-4">
                    ...
                    <div class="col-12">
                        ...
                        <input type="text" id="title" name="title"
                               ...
                               value="{{ old('title', $readingMaterial->title) }}" required>
                        @error('title')
                        ...
                        <div class="d-flex gap-2">
                            <select id="author_id" name="author_id"
                                ...
                                @foreach ($authors as $author)
                                    <option value="{{ $author->id }}" @selected(old('author_id', $readingMaterial->author_id) == $author->id)>
                                        {{ $author->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <div class="d-flex gap-2">
                                <select id="category_id" name="category_id"
                                    ...
                                    @foreach ($categories as $category)
                                        <option value="{{ $category->id }}" @selected(old('category_id', $readingMaterial->category_id) == $category->id)>
                                            {{ $category->name }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <select id="status" name="status"
                            ...
                            @foreach ($statuses as $status)
                                <option value="{{ $status->value }}" @selected(old('status', $readingMaterial->status->value) == $status->value)>
                                    {{ $status->value }}
                                </option>
                            @endforeach
                        </select>
